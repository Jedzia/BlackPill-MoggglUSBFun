...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
      ...
      extern uint8_t press_report[PRESS_REPORT_SIZE];

      //MyLED pin refers to the LED connected to "PC12".
      HAL_GPIO_TogglePin(BUILTIN_LED_GPIO_Port, BUILTIN_LED_Pin); //Toggle LED
      HAL_Delay(150); //Delay 300 milliseconds

      sendMessage(index);
      /*press_report[0] = KEY_MOD_LSHIFT;
      press_report[2] = KEY_X; // send button press
